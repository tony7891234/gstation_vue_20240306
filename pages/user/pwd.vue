<template>
	<view id="pwd">
		<c-head :title="$t('pwd.title')"></c-head>
		<view class="form">
			<u-form :model="form" ref="uForm">
				<u-form-item prop="password" label-position="top" :border-bottom="false">
					<u-input :border="true" class="input_type" v-model="form.password" :placeholder="$t('pwd.placeholder1')" type="password" :clearable="false"/>
				</u-form-item>
				<u-form-item prop="new_password" label-position="top" :border-bottom="false">
					<u-input :border="true" class="input_type" v-model="form.new_password" :placeholder="$t('pwd.placeholder2')" type="password" :clearable="false"/>
				</u-form-item>
				<u-form-item prop="confirm_pwd" label-position="top" :border-bottom="false">
					<u-input :border="true" class="input_type" v-model="form.confirm_pwd" :placeholder="$t('pwd.placeholder3')" type="password" :clearable="false"/>
				</u-form-item>
			</u-form>
...
<script>
	...
	import { changePwdServ } from '@/service/user.api.js'
		...
		computed: {
			rules() {
				return {
					...
					new_password: [
						{
							message: this.$t('auth.tips.new_pwd'), // 必须包含数字字母的组合
							// 可以单个或者同时写两个触发验证方式 ['change','blur']
							trigger: 'change',
							validator: (rule, value, callback) => {
								return value.length >= 6
		...
		methods: {
			...
			handleChangePwd() {
				this.$refs.uForm.validate(valid => {
					if (valid) {
						changePwdServ(this.form).then(res => {
							if (res.code === 0) {
								this.form = {}
								uni.showToast({
									icon: 'none',
